extends RefCounted
## 制作工具拥有的旧 Schema 1–11 导入边界；只转换 JSON，不依赖旧引擎。
...
## 已退出当前内容的技能只保留历史身份；停止支持 Schema 1–11 时随导入器删除。
const RETIRED_MAIN_ABILITY_IDS: Array[String] = ["SK022", "SK032", "SK069", "SK070", "SK071", "SK072", "SK073", "SK074"]
...
func _build(old: Dictionary) -> Dictionary:
	...
	for card in _rows(old.get("cards"), "章节卡牌"):
		...
		if kind == 3:
			if not _is_legacy_main_ability(definition_id) or copies < 1 or copies > 9:
				error = "旧技能卡引用或融合份数损坏。"
				continue
	...
	for slot in _rows(market.get("slots"), "市场槽位"):
		...
		if offer_id.begins_with("ability:"):
			if not _is_legacy_main_ability(offer_id.trim_prefix("ability:")):
				error = "旧市场包含未知技能。"
				continue
...
## 旧卡牌与市场只核验身份再折算奖励，不为兼容恢复退役技能数值。
func _is_legacy_main_ability(id: String) -> bool:
	var alias: Dictionary = content.get_record("ability_aliases", id)
	var canonical_id: String = str(alias.get("ability_id", id))
	return canonical_id in RETIRED_MAIN_ABILITY_IDS or not content.get_record("main_abilities", canonical_id).is_empty()
