...
static func create_fake_javascript_object() -> Variant:
	# MagicA: non-TT Web builds still initialize the SDK autoload; an empty
	# constructor name throws in JavaScript. Native builds need no JS object.
	return JavaScriptBridge.create_object("Object") if OS.has_feature("web") else null
