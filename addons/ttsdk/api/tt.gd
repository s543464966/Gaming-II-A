...
func create_video_stream(url: String) -> VideoStream:
	if ClassDB.class_exists("TTVideoStream"):
		var stream: VideoStream = ClassDB.instantiate("TTVideoStream")
		stream.set_file(url)
		return stream

	if url.get_extension().to_lower() == "ogv":
		var stream := VideoStreamTheora.new()
		stream.set_file(url)
		return stream

	var stream: VideoStream = load("res://addons/ttsdk/mock/video.gd").new()
	stream.set_file(url)
	return stream
...
func _enter_tree() -> void:
	# MagicA: native/Web/WeChat previews must not subscribe to absent TT APIs.
	if not is_run_in_tt(): return
