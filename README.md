...
| `features/player_session/` | 本机默认玩家创建与恢复、保存编排及现行存档升级 |
...
旧 Schema 1–11 的导入器、转换器及退役技能身份识别统一归 [tooling/player_data](tooling/player_data/README.md)，不参与游戏运行或导出。现行存档升级和启动时旧身份恢复仍归 `features/player_session/`，不会因导入工具迁移而移除。
...
- 素材按当前消费者和明确备用用途收敛；现用字体及对应许可证保留，已退役的旧界面、骰面和逐技能动画不再常驻工程。六类命中序列与备用弹道仍分别维护。
